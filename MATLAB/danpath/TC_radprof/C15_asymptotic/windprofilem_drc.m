function [ V ] = windprofilem_drc( vm, rm, vm2, rm2, r, wp, fcor)
...
elseif wprofile == 4  % (Experimental; not recommended)
    ...
    %
    r0=rm.*sqrt(chi.*vm./vp).*((16.*vm./(1000.*fcor.*rm)).^(1/3)-1).^0.75;  %DRC: this assumes the high-vorticity limit (gamma<<1)
    ra=(rm.*r0.^2.*vp./(chi*vm)).^(1/3);    %DRC: this assumes the high-vorticity limit (gamma<<1)
    va=2.*vm.*(ra./rm)./(1+(ra./rm).^2);    %DRC: this assumes the high-vorticity limit (gamma<<1)
    u=max(sign(ra-r),0);
    m1=2.*rm.*vm.*(r./rm).^2./(1+(r./rm).^2); % Inner (ER2011) profile %DRC: this assumes the high-vorticity limit (gamma<<1)
