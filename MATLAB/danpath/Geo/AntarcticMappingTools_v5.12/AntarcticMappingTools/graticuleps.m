...
h = plotps(lat,lon,varargin{:}); 

% Change color to gray unless user specified something else: 
% This will not work if user wants a pure blue graticule, but whom in their right mind wants a pure blue graticule?  
col = get(h,'color');
if(isequal(col,[0 0 1]) && ~any(strncmpi(varargin,'col',3)))
    set(h,'color',.5*[1 1 1]);
end
