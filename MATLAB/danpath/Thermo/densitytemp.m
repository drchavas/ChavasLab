function [Trho] = densitytemp(T,r,rl,ri)
% [Trho] = densitytemp(T,r,rl,ri)
% Calculates the density temperature, Emanuel Eq. 4.3.5
% Temperature (K)
% water vapor mixing ratio (r) [kg/kg]
% liquid water mixing ratio (rl) [kg/kg]
% ice water mixing ratio (ri) [kg/kg]

    %% Get all the constants we need
    const = CM1_constants;

    rtot = r+rl+ri;
    Trho = T.*(1+r/const.eps)./(1+rtot);
