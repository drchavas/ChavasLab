          function [PMIN,VMAX,AIRSEA,TO,IFL]= pcmin_emanuel_ver20141030(SST,PSL,P,T,R)
%
...
%
%           AIRSEA is the air-sea entropy disequilibrium [J/kg/K]
%
...
%
	CKCD=0.7;   %default is 0.9; BE02 uses 0.9; TE12 uses 0.7
%
...
%
	VREDUC=0.75; %default = 0.8; BE02 uses 0.9; TE12 uses 0.75
%
%--------------------------------------------------------------------------
%
	SSTK=SST+273.15;
...
%
    if SST <= 5.0
	 ...
	 PMIN=0.0;
	 IFL=0;
     TO=0.0;    %DRC 2015-08-05
     AIRSEA = -1;   %DRC 2015-08-24 from pcmin_TE12.m
	 return
    end
%
...
%
      while (abs(PNEW-PMOLD)) > 0.5 %DRC 2015-08-24 this threshold doesnt affect result
        %
        ...
        %
              TP=SSTK;
              %% DRC 2016-01-25 -- calculate surface saturation mixing ratio using background sea level pressure
              %PP=min(PM,1000.0);
              PP=PSL;
              %% END DRC 2016-01-25
              RP=0.622.*ES0./(PP-ES0);
...
%
	 ...
	 VMAX=sqrt(CKCD.*RAT.*FAC); %moved VREDUC to end DRC 2015-08-24
     
     %Calculate air-sea disequilibrium -- DRC 2015-08-24
    AIRSEA = VMAX^2*(1/CKCD)*TO/(SSTK*(SSTK-TO));   %solved directly from PI equation
    
    %%Reduce Vmax to z=10m
    VMAX=VREDUC.*VMAX;  %DRC 2015-08-24
%
%  
