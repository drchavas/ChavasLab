          function [PMIN,VMAX,TOM,IFL]= pcmin_drc20130423(SST,PSL,P,T,R,CKCD,SIG,IDISS,b,NK,VREDUC,ptop)
          
%DRC 2013-04-23: Line 256 -- if statement was added to match the FORTRAN
%code, which now means that their output values match exactly
%updated to match the values in FORTRAN code (line 117 threshold from .5 to .; line 101 PM from 930 to 950)
...
%
      ...
      RP=R(NK);
      PP=P(NK);
      [CAPEA, ~, IFLAG]= cape(TP,RP,PP,T,R,P,SIG,ptop);
      if IFLAG ~= 1
          IFL=2;
      end    
%
...
%
      TP=T(NK);
      PP=min(PM,1000.0);
      %%pp differs on 2nd iter, so does CAPEM, CAPEMS
      RP=0.622.*R(NK).*PSL./(PP.*(0.622+R(NK))-R(NK).*PSL);
      
      [CAPEM, TOM, IFLAG]=cape(TP,RP,PP,T,R,P,SIG,ptop);
      if IFLAG ~= 1
          IFL=2;
      end    
%
...
%
      ...
      PP=min(PM,1000.0);
      RP=0.622.*ES0./(PP-ES0);
      [CAPEMS, TOMS, IFLAG]=cape(TP,RP,PP,T,R,P,SIG,ptop);
      if IFLAG ~= 1
          IFL=2;
      end    
      RAT=SSTK/TOMS;
...
%
    ...
    %TV1 same, SSTK same, RS0 different
	CAT=CAPEM-CAPEA+0.5.*CKCD.*RAT.*(CAPEMS-CAPEM);
    %between 2nd and 3rd step, RAT and CAPEMS jump for 9.83 relative to
    %changes between 9.81 and 9.82; CAPEM changes smoothly
    drc(iter,:) = [CAT CAPEM 10*CAPEA 1000*CKCD 10000*RAT CAPEMS];
    
	CAT=max(CAT,0.0);
	PNEW=PSL.*exp(-CAT./(287.04.*TVAV));
    
    %CAT different right away
...
%      
     ...
     %CAPEM, CAPEMS drops significantly when going from CKCD = 9.82 to 9.83
	 CAT=max(CAT,0.0);
...
%
%  
     function [CAPED,TOB,IFLAG]= cape(TP,RP,PP,T,R,P,SIG,ptop)
%
...
%-------------------------------------------------------------------------
      %ptop=20;   %  Pressure below which sounding is ignored
%------------------------------------------------------------------------      
      Nold=max(size(P));
      N=1;
      for i=Nold:-1:1,
...
%
      CPD=1005.7;
      CPV=1870.0;
%     CL=4190.0;    %4190 is the correct value; this is a fudge factor to account for fact that code
%     does not account for latent heat of fusion (Kerry pers. comm.,
%     2013-04-23); recommends keeping it as is
      CL=2500.0;
      CPVMCL=CPV-CL;
      RV=461.5;
      RD=287.04;
...
%    
    for J=1:N,
%
    if P(J)>ptop && P(J)<PP %DRC 2013-04-23: this if statement was added to match the FORTRAN code, which now means that their output values match exactly.
