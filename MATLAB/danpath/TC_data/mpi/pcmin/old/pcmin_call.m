...
function [Vp_reversible Vp_pseudoadiabatic]= pcmin_call(dir_in,snd_file,Tsst,CKCD_in,IDISS_in,b_in,NK_in,VREDUC_in,ptop)
...
%% Extract initial sounding; note: pp00 is only the hydrostatically-balanced pressure associated with th00, qv00, p_sfc (this is the background state CM1 calculates)
[zz00 pp00 th00 qv00 u00 v00 T00 Tv00 thv00 rho00 qvs00 rh00 pi00 p_sfc th_sfc qv_sfc] = snd_extract_nointerpolate(dir_in,snd_file);

%% To obtain the full pressure field associated with the sounding, must extract data from the corresponding *_pressures file
[pp00_full] = snd_pressures_extract_nointerpolate(dir_in,snd_file);
pp00_full = pp00_full';
