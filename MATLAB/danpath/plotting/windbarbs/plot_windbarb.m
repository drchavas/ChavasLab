...
sense = sign(lat).*ones(size(u)); % check for hemisphere and set clockwise/anticlockwise feathers

% convert u,v to azimuth and speed:
azmth = atan2(u,v);
azmth = azmth+pi;     %ADDED DRC 09/2009
speed = sqrt(u.^2 + v.^2);
...
for ibarb = 1:nbarbs
     ...
     if nflag+nfull+nhalf == 0 % no feathers:
         % just plot a point:
         barb_patch = [];
         %disp(['Barb #',int2str(ibarb),': (rounded) speed = 0: simply
            %drawing a point']);
     elseif nhalf==1 & nflag+nfull==0 % only a 5 m/s feather:
         % place the half-feather in the second feather spot, not at the end:
         barb_patch = [0 0;0 shaft_length;0 shaft_length-feather_sep;half_length.*cos(feather_ang) half_length.*sin(feather_ang)+shaft_length-feather_sep;0 shaft_length-feather_sep]*clockmat;         %disp(['Barb #',int2str(ibarb),': (rounded) speed = 5; drawing half-feather in from the end']);
     else % all other cases:
         ifeather = 0;
         barb_patch = [0 0;0 shaft_length];
         %disp(['Barb #',int2str(ibarb),': (rounded) speed = ',int2str(speed(ibarb))]);
         for iflag = 1:nflag
             flag_patch = [0 0;full_length.*cos(feather_ang) 0;0 0-feather_sep]*clockmat + repmat([0 shaft_length-ifeather.*feather_sep],3,1);
             barb_patch = [barb_patch;flag_patch];
             ifeather=ifeather+1;
             %disp([' ',int2str(ifeather),': drawing a 50 m/s flag']);
         end
         for ifull = 1:nfull
             full_patch = [0 0;full_length.*cos(feather_ang) full_length.*sin(feather_ang);0 0]*clockmat + repmat([0 shaft_length-ifeather.*feather_sep],3,1);
             barb_patch = [barb_patch;full_patch];
             ifeather=ifeather+1;
             %disp([' ',int2str(ifeather),': drawing a 10 m/s feather']);
         end
         if nhalf == 1
             half_patch = [0 0;half_length.*cos(feather_ang) half_length.*sin(feather_ang);0 0]*clockmat + repmat([0 shaft_length-ifeather.*feather_sep],3,1);
