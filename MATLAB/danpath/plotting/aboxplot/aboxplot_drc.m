...
function aboxplot_drc(X,varargin)
...
for j=1:d
    ...
    for i=1:n
        ...
        nhi = q2 + 1.57*(q3-q1)/sqrt(nx);
        if ~isfinite(nhi)
            nhi = q2;
        end
        nlo = q2 - 1.57*(q3-q1)/sqrt(nx);
        if ~isfinite(nlo)
            nlo = q2;
        end
        %% end DRC EDIT 2014-11-03 %%%%%%%%%%%%%%%%%%%%%%%%
        ...
        %% DRC EDIT 2014-11-03 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Plot notch range for median
        rectangle('Position',[i-widthl/2+(2*j-1-widths)*widthl/(2*d) nlo widths*widthl/d nhi-nlo],...
            'EdgeColor',[.5 0 0],'FaceColor','none','HitTest','off','Parent',hgg(j));
...
%             'Color','k','LineWidth',1,'HitTest','off','Parent',hgg(j));
        ...
        % Plot median
        line([i-widthl/2+(2*j-1-widths)*widthl/(2*d) i-widthl/2+(2*j-1+widths)*widthl/(2*d)],[q2 q2],...
            'Color','r','LineWidth',1,'HitTest','off','Parent',hgg(j));                
